...
import 'package:jam/calendar_widget.dart';

void main() {
  testWidgets('CalendarWidget renders correctly', (WidgetTester tester) async {
    // Build our widget
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: CalendarWidget())));
  ...
  testWidgets('CalendarWidget displays current month', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: CalendarWidget())));
  ...
  testWidgets('CalendarWidget allows day selection', (WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: CalendarWidget())));
    
    // Find a day cell that's not today (to avoid flaky tests)
    final today = DateTime.now();
    final testDay = today.day == 15 ? today.add(Duration(days: 1)) : DateTime(today.year, today.month, 15);
...
testWidgets('CalendarWidget allows month navigation', (WidgetTester tester) async {
  await tester.pumpWidget(MaterialApp(home: Scaffold(body: CalendarWidget())));
  
  final initialMonth = DateTime.now();
  final initialMonthText = '${_getMonthName(initialMonth.month)} ${initialMonth.year}';
